...
function checkingWords(pieces, words, score) {
    let player = p1IsNext ? p1 : p2;
        ...
        mainWord = [...words][0],
        fetchMainWord = () => {
            fetch(`https://kateglo.com/api.php?format=json&phrase=${mainWord}`)
    ...
    words.map(word => {
        promisesOfWords.push(fetch(`https://kateglo.com/api.php?format=json&phrase=${word}`).then(response => response.json()))
    });
